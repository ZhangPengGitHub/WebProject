...
function handleJournalDelete() {
	...
	var query = new AV.Query(Journal);
	query.find({
			success : function(results) {
				...
				for ( var i = 0; i < results.length; i++) {
					object[i] = results[i];
					temp[0] = object[i].get('abbreviation');
					temp[1] = object[i].get('wholeName');
				...
				c1.innerHTML = "删除";
				var c2 = r.insertCell();
				c2.innerHTML = "abbreviation";
				var c3 = r.insertCell();
				c3.innerHTML = "wholeName";
				
				document.getElementById('tableJournal').appendChild(t);
