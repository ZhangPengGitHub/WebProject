...
public class Alter extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getSession(false) != null ){
			...
			String[][] studentname = Count.work(un);
			
			int if_plan = 1;

			work.updateifplan(studentname[id][3], if_plan);
			work.updatereason(studentname[id][3], "");
			
			request.getSession().setAttribute("content", Count.work(un));

			String path = request.getContextPath();
			String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
			response.sendRedirect(basePath+"jsp/temp_alter.jsp");
		}
		else{
			String path = request.getContextPath();
			String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
