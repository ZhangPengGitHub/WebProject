...
public class WorkStudent extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getSession(false) != null ){
			...
			ResultSet rs = work.selectWork(username,num);
			ResultSet rs_id = user.selectUser();
			
		    int id = 0;
		    
			try {
				while (rs_id.next()){
					if(!rs_id.getString(1).equals(username)){
						id++;
					}
					else{
						break;
			...
			request.getSession().setAttribute("whatweek",num);
			request.getSession().setAttribute("whatstudent",id);
